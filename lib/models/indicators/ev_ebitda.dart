...
import 'package:fin_chart/ui/indicator_settings/ev_ebitda_settings_dialog.dart';
...
class EvEbitdaPoint {
  ...
  EvEbitdaPoint({required this.date, required this.value});
  ...
  factory EvEbitdaPoint.fromJson(Map<String, dynamic> json) {
    return EvEbitdaPoint(
...
class EvEbitda extends Indicator {
  Color lineColor = Colors.purple;
  List<EvEbitdaPoint> points = [];
  double defaultValue = 15.0; // Add this default
  ...
  EvEbitda({
    ...
    this.points = const [],
    this.defaultValue = 15.0, // Add to constructor
  }) : super(
            id: generateV4(),
            type: IndicatorType.evEbitda,
  ...
  EvEbitda._({
    ...
    this.lineColor = Colors.purple,
    this.points = const [],
    this.defaultValue = 15.0, // Add to private constructor
  });
  ...
  void _calculateValues() {
    ...
    // Sort points by date
    List<EvEbitdaPoint> sortedPoints = List.from(points);
  ...
  showIndicatorSettings(
    ...
    showDialog(
      context: context,
      builder: (context) => EvEbitdaSettingsDialog(
        indicator: this,
        onUpdate: onUpdate,
      ),
    );
  ...
  factory EvEbitda.fromJson(Map<String, dynamic> json) {
    return EvEbitda._(
      ...
      points: (json['points'] as List?)
              ?.map((p) => EvEbitdaPoint.fromJson(p))
              .toList() ??
          [],
      defaultValue: json['defaultValue']?.toDouble() ?? 15.0, // Add this
    );
  }
