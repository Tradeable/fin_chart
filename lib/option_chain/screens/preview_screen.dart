...
class PreviewScreenState extends State<PreviewScreen> {
  ...
  List<DataCell> _buildRowCells(int rowIndex, OptionData data) {
  ...
  Widget _buildCellContent(OptionData data, ColumnType columnType) {
  ...
  int? _getStrikeColumnIndex() {
    final visibleColumns =
        widget.previewData.columns.where((c) => c.visible).toList();
    for (int i = 0; i < visibleColumns.length; i++) {
      if (visibleColumns[i].type == ColumnType.strike) {
        return i;
      }
    }
    return null;
